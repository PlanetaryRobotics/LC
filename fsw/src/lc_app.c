...
void LC_AppMain(void) {
    ...
    */
    while (CFE_ES_RunLoop(&RunStatus) == true) {
        ...
        */
        Status = CFE_SB_RcvMsg(&LC_OperData.MsgPtr, LC_OperData.CmdPipe,
                               CFE_SB_PEND_FOREVER);
...
int32 LC_AppInit(void) {
    ...
    */
    CFE_EVS_SendEvent(LC_INIT_INF_EID, CFE_EVS_EventType_INFORMATION,
                      "LC Initialized. Version %d.%d.%d.%d", LC_MAJOR_VERSION,
    ...
    */
    ...
    if (Status != CFE_SUCCESS) {
        CFE_ES_WriteToSysLog(
            "LC App: Error Registering For Event Services, RC = 0x%08X\n",
            (unsigned int)Status);
        return (Status);
    }
...
int32 LC_SbInit(void) {
    ...
    */
    CFE_SB_InitMsg(&LC_OperData.HkPacket, LC_HK_TLM_MID, sizeof(LC_HkPacket_t),
                   false);
    ...
    */
    Status =
        CFE_SB_CreatePipe(&LC_OperData.CmdPipe, LC_PIPE_DEPTH, LC_PIPE_NAME);
    if (Status != CFE_SUCCESS) {
    ...
    */
    ...
    if (Status != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            LC_SUB_HK_REQ_ERR_EID, CFE_EVS_EventType_ERROR,
    ...
    */
    ...
    if (Status != CFE_SUCCESS) {
        CFE_EVS_SendEvent(
            LC_SUB_SAMPLE_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
...
/* lc_platform_cfg.h */
    ...
    */
    ...
    /*
    ** If any CDS area or critical table is not restored - initialize
    *everything.
    **  (might be due to reset type, CDS disabled or corrupt, table restore
    *error)
    */
    if (((LC_OperData.TableResults & LC_WRT_CDS_RESTORED) ==
         LC_WRT_CDS_RESTORED) &&
        ...
        */
        Result = CFE_TBL_GetAddress((void *)&LC_OperData.WDTPtr,
                                    LC_OperData.WDTHandle);
        ...
        */
        Result = CFE_TBL_GetAddress((void *)&LC_OperData.ADTPtr,
                                    LC_OperData.ADTHandle);
    ...
    */
    if ((LC_OperData.TableResults & LC_CDS_ENABLED) == LC_CDS_ENABLED) {
        if ((LC_OperData.TableResults & LC_CDS_RESTORED) == LC_CDS_RESTORED) {
            CFE_EVS_SendEvent(
                LC_CDS_RESTORED_INF_EID, CFE_EVS_EventType_INFORMATION,
                "Previous state restored from Critical Data Store");
        } else if ((LC_OperData.TableResults & LC_CDS_UPDATED) ==
                   LC_CDS_UPDATED) {
            CFE_EVS_SendEvent(LC_CDS_UPDATED_INF_EID,
                              CFE_EVS_EventType_INFORMATION,
                              "Default state loaded and written to CDS, "
                              "activity mask = 0x%08X",
                              (unsigned int)LC_OperData.TableResults);
        }

    } else {
        CFE_EVS_SendEvent(
            LC_CDS_DISABLED_INF_EID, CFE_EVS_EventType_INFORMATION,
...
int32 LC_CreateResultTables(void) {
    ...
    */
    ...
    Result =
        CFE_TBL_GetAddress((void *)&LC_OperData.WRTPtr, LC_OperData.WRTHandle);
    ...
    */
    ...
    Result =
        CFE_TBL_GetAddress((void *)&LC_OperData.ARTPtr, LC_OperData.ARTHandle);
...
int32 LC_CreateDefinitionTables(void) {
    ...
    /*
    ** If CDS is still enabled, try to register the 2 definition tables as
    *critical
    **  (if error, continue with init - but disable CDS and re-register as
    *non-critical)
    */
    if (LC_OperData.HaveActiveCDS) {
    ...
    */
    if (((LC_OperData.TableResults & LC_WDT_CRITICAL_TBL) ==
         LC_WDT_CRITICAL_TBL) &&
        ((LC_OperData.TableResults & LC_ADT_NOT_CRITICAL) ==
         LC_ADT_NOT_CRITICAL)) {
        /*
        ...
        */
        ...
        if (Result == CFE_SUCCESS) {
            ...
            */
            CFE_EVS_SendEvent(
                LC_WDT_REREGISTER_ERR_EID, CFE_EVS_EventType_ERROR,
...
int32 LC_CreateTaskCDS(void) {
    ...
    */
    DataSize = LC_MAX_WATCHPOINTS * sizeof(LC_WRTEntry_t);
    Result   = CFE_ES_RegisterCDS(&LC_OperData.WRTDataCDSHandle, DataSize,
                                LC_WRT_CDSNAME);

    if (Result == CFE_SUCCESS) {
        /*
        ** Normal result after a power on reset (cold boot) - continue with next
        *CDS area
        */
        LC_OperData.TableResults |= LC_WRT_CDS_CREATED;
    } else if (Result == CFE_ES_CDS_ALREADY_EXISTS) {
        /*
        ** Normal result after a processor reset (warm boot) - try to restore
        *previous data
        */
        LC_OperData.TableResults |= LC_WRT_CDS_CREATED;

        Result = CFE_ES_RestoreFromCDS(LC_OperData.WRTPtr,
                                       LC_OperData.WRTDataCDSHandle);
    ...
    */
    DataSize = LC_MAX_ACTIONPOINTS * sizeof(LC_ARTEntry_t);
    Result   = CFE_ES_RegisterCDS(&LC_OperData.ARTDataCDSHandle, DataSize,
                                LC_ART_CDSNAME);
    ...
    if (Result == CFE_SUCCESS) {
        ...
        */
        ...
        Result = CFE_ES_RestoreFromCDS(LC_OperData.ARTPtr,
                                       LC_OperData.ARTDataCDSHandle);
    ...
    */
    ...
    Result   = CFE_ES_RegisterCDS(&LC_OperData.AppDataCDSHandle, DataSize,
                                LC_APPDATA_CDSNAME);
    ...
    if (Result == CFE_SUCCESS) {
        ...
        */
        ...
        Result =
            CFE_ES_RestoreFromCDS(&LC_AppData, LC_OperData.AppDataCDSHandle);
        ...
        CFE_EVS_SendEvent(
            LC_APP_CDS_REGISTER_ERR_EID, CFE_EVS_EventType_ERROR,
...
int32 LC_LoadDefaultTables(void) {
    ...
    */
    Result =
        CFE_TBL_Load(LC_OperData.WDTHandle, CFE_TBL_SRC_FILE, LC_WDT_FILENAME);
    ...
    */
    Result =
        CFE_TBL_Load(LC_OperData.ADTHandle, CFE_TBL_SRC_FILE, LC_ADT_FILENAME);
