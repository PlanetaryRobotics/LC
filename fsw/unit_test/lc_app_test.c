...
**
** Purpose:
**   This file contains unit test cases for the functions contained in the file
*lc_app.c
**
** References:
...
int32 LC_APP_TEST_CFE_TBL_RegisterHook1(
    CFE_TBL_Handle_t *TblHandlePtr, const char *Name, uint32 Size,
...
int32 LC_APP_TEST_CFE_TBL_RegisterHook2(
    CFE_TBL_Handle_t *TblHandlePtr, const char *Name, uint32 Size,
    uint16 TblOptionFlags, CFE_TBL_CallbackFuncPtr_t TblValidationFuncPtr) {
    LC_APP_TEST_CFE_TBL_RegisterHookCount++;
...
int32 LC_APP_TEST_CFE_TBL_RegisterHook3(
    CFE_TBL_Handle_t *TblHandlePtr, const char *Name, uint32 Size,
    uint16 TblOptionFlags, CFE_TBL_CallbackFuncPtr_t TblValidationFuncPtr) {
    LC_APP_TEST_CFE_TBL_RegisterHookCount++;
...
int32 LC_APP_TEST_CFE_TBL_RegisterHook4(
    CFE_TBL_Handle_t *TblHandlePtr, const char *Name, uint32 Size,
    uint16 TblOptionFlags, CFE_TBL_CallbackFuncPtr_t TblValidationFuncPtr) {
    LC_APP_TEST_CFE_TBL_RegisterHookCount++;
...
#ifndef LC_SAVE_TO_CDS /* When this is not defined EVENT ID=23 is always \
                          issued (LC use of Critical Data Store disabled) */
...
void LC_AppMain_Test_RegisterAppError(void) {
    ...
    /* Set to satisfy condition "Status != CFE_SUCCESS" immediately following
     * call to CFE_ES_RegisterApp */
    ...
    /* Verify results */
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_TASK_EXIT_EID, CFE_EVS_CRITICAL,
...
void LC_AppMain_Test_AppInitError(void) {
    ...
    /* Set to make subfunction LC_AppInit return -1, in order to fail subsequent
     * condition "Status != CFE_SUCCESS" */
...
void LC_AppMain_Test_RcvMsgError(void) {
    ...
    /* Set to fail condition "Status != CFE_SUCCESS" immediately following call
     * to CFE_SB_RcvMsg */
...
void LC_AppInit_Test_Nominal(void) {
    ...
    /* Verify results */
    sprintf(Message, "LC Initialized. Version %d.%d.%d.%d", LC_MAJOR_VERSION,
            LC_MINOR_VERSION, LC_REVISION, LC_MISSION_REV);
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_INIT_INF_EID, CFE_EVS_INFORMATION, Message),
        Message);
    ...
    /* Note: not testing LC_OperData.WDTPtr == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.ADTPtr == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.WRTPtr == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.ARTPtr == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.WDTHandle == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.ADTHandle == 0, because it is modified by a
     * subfunction */
    UtAssert_True(LC_OperData.WRTHandle == 0, "LC_OperData.WRTHandle == 0");
    /* Note: not testing LC_OperData.ARTHandle == 0, because it is modified by a
     * subfunction */
    UtAssert_True(LC_OperData.WRTDataCDSHandle == 0,
                  "LC_OperData.WRTDataCDSHandle == 0");
    UtAssert_True(LC_OperData.ARTDataCDSHandle == 0,
                  "LC_OperData.ARTDataCDSHandle == 0");
    UtAssert_True(LC_OperData.AppDataCDSHandle == 0,
                  "LC_OperData.AppDataCDSHandle == 0");
    /* Note: not testing LC_OperData.WatchpointCount == 0, because it is
     * modified by a subfunction */
    /* Note: not testing LC_OperData.MessageIDsCount == 0, because it is
     * modified by a subfunction */
    /* Note: not testing LC_OperData.HkPacket == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.TableResults == 0, because it is modified
     * by a subfunction */
    /* Note: not testing LC_OperData.HashTable == 0, because it is modified by a
     * subfunction */
    /* Note: not testing LC_OperData.MessageLinks == 0, because it is modified
     * by a subfunction */
    /* Note: not testing LC_OperData.WatchPtLinks == 0, because it is modified
     * by a subfunction */
    UtAssert_True(LC_OperData.HaveActiveCDS == 0,
                  "LC_OperData.HaveActiveCDS == 0");

    UtAssert_True(LC_AppData.CmdCount == 0, "LC_AppData.CmdCount == 0");
    UtAssert_True(LC_AppData.CmdErrCount == 0, "LC_AppData.CmdErrCount == 0");
    UtAssert_True(LC_AppData.APSampleCount == 0,
                  "LC_AppData.APSampleCount == 0");
    UtAssert_True(LC_AppData.MonitoredMsgCount == 0,
                  "LC_AppData.MonitoredMsgCount == 0");
    UtAssert_True(LC_AppData.RTSExecCount == 0, "LC_AppData.RTSExecCount == 0");
    UtAssert_True(LC_AppData.PassiveRTSExecCount == 0,
                  "LC_AppData.PassiveRTSExecCount == 0");
    UtAssert_True(LC_AppData.CDSSavedOnExit == 0,
                  "LC_AppData.CDSSavedOnExit == 0");
    /* Note: not testing LC_AppData.CurrentLCState == 0, because it is modified
     * by a subfunction */
...
void LC_AppInit_Test_EVSInitError(void) {
    ...
    /* Set to cause LC_EvsInit to return -1, in order to satisfy subsequent
     * condition "Status != CFE_SUCCESS" */
    ...
    UtAssert_True(Ut_CFE_EVS_GetEventQueueDepth() == 0,
                  "Ut_CFE_EVS_GetEventQueueDepth() == 0");
...
void LC_AppInit_Test_SBInitError(void) {
    ...
    /* Set to cause LC_SbInit to return -1, in order to satisfy subsequent
     * condition "Status != CFE_SUCCESS" */
    Ut_CFE_SB_SetReturnCode(UT_CFE_SB_CREATEPIPE_INDEX, -1, 1);
...
void LC_AppInit_Test_TableInitError(void) {
    ...
    /* Set to cause LC_TableInit to return -1, in order to satisfy subsequent
     * condition "Status != CFE_SUCCESS" */
...
void LC_EvsInit_Test_EVSRegisterError(void) {
    ...
    /* Set to generate system log message "LC App: Error Registering For Event
     * Services" */
    Ut_CFE_EVS_SetReturnCode(UT_CFE_EVS_REGISTER_INDEX, -1, 1);
...
void LC_SbInit_Test_CreatePipeError(void) {
    ...
    /* Verify results */
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_CR_PIPE_ERR_EID, CFE_EVS_ERROR,
...
void LC_SbInit_Test_SubscribeHKReqError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_SUB_HK_REQ_ERR_EID, CFE_EVS_ERROR,
...
void LC_SbInit_Test_SubscribeGndCmdError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_SUB_GND_CMD_ERR_EID, CFE_EVS_ERROR,
...
void LC_SbInit_Test_SubscribeSampleCmdError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_SUB_SAMPLE_CMD_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateResultTables_Test_WRTRegisterError(void) {
    ...
    /* Verify results */
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_WRT_REGISTER_ERR_EID, CFE_EVS_ERROR,
                                       "Error registering WRT, RC=0xFFFFFFFF"),
                  "Error registering WRT, RC=0xFFFFFFFF");

    UtAssert_True(LC_OperData.TableResults == 0,
                  "LC_OperData.TableResults == 0");
    UtAssert_True(Result == -1, "Result == -1");
...
void LC_CreateResultTables_Test_WRTGetAddressError(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_WRT_GETADDR_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateResultTables_Test_ARTRegisterError(void) {
    ...
    /* Verify results */
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_ART_REGISTER_ERR_EID, CFE_EVS_ERROR,
                                       "Error registering ART, RC=0xFFFFFFFF"),
                  "Error registering ART, RC=0xFFFFFFFF");

    UtAssert_True(LC_OperData.TableResults == LC_WRT_TBL_CREATED,
                  "LC_OperData.TableResults == LC_WRT_TBL_CREATED");
    UtAssert_True(Result == -1, "Result == -1");
...
void LC_CreateResultTables_Test_ARTGetAddressError(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_ART_GETADDR_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateDefinitionTables_Test_WDTReRegisterThenInfoRecovered(void) {
    ...
    /* Satisfies condition "(LC_OperData.HaveActiveCDS) && ((Result !=
       CFE_TBL_INFO_RECOVERED_TBL) && (Result != CFE_SUCCESS))" and then
       satisfies condition "Result == CFE_TBL_INFO_RECOVERED_TBL" */
    LC_APP_TEST_CFE_TBL_RegisterHookCount = 0;
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_REGISTER_INDEX,
                               &LC_APP_TEST_CFE_TBL_RegisterHook1);
    ...
    /* Verify results */
    UtAssert_True(Result == CFE_SUCCESS, "Result == CFE_SUCCESS");
    UtAssert_True(LC_OperData.HaveActiveCDS == FALSE,
                  "LC_OperData.HaveActiveCDS == FALSE");
    UtAssert_True(
        LC_OperData.TableResults == (LC_WDT_CRITICAL_TBL | LC_WDT_TBL_RESTORED |
                                     LC_ADT_CRITICAL_TBL | LC_ADT_TBL_RESTORED),
        "LC_OperData.TableResults == (LC_WDT_CRITICAL_TBL | "
        "LC_WDT_TBL_RESTORED | LC_ADT_CRITICAL_TBL | LC_ADT_TBL_RESTORED)");
...
void LC_CreateDefinitionTables_Test_ADTReRegisterThenInfoRecovered(void) {
    ...
    LC_APP_TEST_CFE_TBL_RegisterHookCount = 0;
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_REGISTER_INDEX,
                               &LC_APP_TEST_CFE_TBL_RegisterHook2);
...
void LC_CreateDefinitionTables_Test_WDTCriticalADTNoncritical(void) {
    ...
    /* Satisfies condition "((LC_OperData.TableResults & LC_WDT_CRITICAL_TBL) ==
       LC_WDT_CRITICAL_TBL) &&
       ((LC_OperData.TableResults & LC_ADT_NOT_CRITICAL) ==
       LC_ADT_NOT_CRITICAL)" */
    LC_APP_TEST_CFE_TBL_RegisterHookCount = 0;
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_REGISTER_INDEX,
                               &LC_APP_TEST_CFE_TBL_RegisterHook3);
    ...
    UtAssert_True(
        LC_OperData.TableResults == (LC_WDT_CRITICAL_TBL | LC_WDT_TBL_RESTORED |
                                     LC_ADT_NOT_CRITICAL | LC_WDT_NOT_CRITICAL),
        "LC_OperData.TableResults == (LC_WDT_CRITICAL_TBL | "
        "LC_WDT_TBL_RESTORED | LC_ADT_NOT_CRITICAL | LC_WDT_NOT_CRITICAL)");
...
void LC_CreateDefinitionTables_Test_WDTReRegisterError(void) {
    ...
    /* Satisfies condition "((LC_OperData.TableResults & LC_WDT_CRITICAL_TBL) ==
       LC_WDT_CRITICAL_TBL) &&
       ((LC_OperData.TableResults & LC_ADT_NOT_CRITICAL) ==
       LC_ADT_NOT_CRITICAL)" and then FAILS condition "Result == CFE_SUCCESS",
       in order to generate error message LC_WDT_REREGISTER_ERR_EID */
    LC_APP_TEST_CFE_TBL_RegisterHookCount = 0;
    Ut_CFE_TBL_SetFunctionHook(UT_CFE_TBL_REGISTER_INDEX,
                               &LC_APP_TEST_CFE_TBL_RegisterHook4);
    ...
    UtAssert_True(
        LC_OperData.TableResults ==
            (LC_WDT_CRITICAL_TBL | LC_WDT_TBL_RESTORED | LC_ADT_NOT_CRITICAL),
        "LC_OperData.TableResults == (LC_WDT_CRITICAL_TBL | "
        "LC_WDT_TBL_RESTORED | LC_ADT_NOT_CRITICAL)");

    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_WDT_REREGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateDefinitionTables_Test_WDTRegisterError(void) {
    ...
    /* Verify results */
    UtAssert_True(Result == -1, "Result == -1");
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_WDT_REGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateDefinitionTables_Test_ADTRegisterError(void) {
    ...
    /* Verify results */
    UtAssert_True(Result == -1, "Result == -1");
    UtAssert_True(Ut_CFE_EVS_EventSent(LC_ADT_REGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateTaskCDS_Test_NominalPowerOnReset(void) {
    ...
    UtAssert_True(
        LC_OperData.TableResults ==
            (LC_WRT_CDS_CREATED | LC_ART_CDS_CREATED | LC_APP_CDS_CREATED),
        "LC_OperData.TableResults == (LC_WRT_CDS_CREATED | LC_ART_CDS_CREATED "
        "| LC_APP_CDS_CREATED)");
...
void LC_CreateTaskCDS_Test_NominalProcessorReset(void) {
    ...
    /* Set to satisfy all instances of condition "Result ==
     * CFE_ES_CDS_ALREADY_EXISTS" */
    Ut_CFE_ES_SetReturnCode(UT_CFE_ES_REGISTERCDS_INDEX,
                            CFE_ES_CDS_ALREADY_EXISTS, 1);
    ...
    UtAssert_True(
        LC_OperData.TableResults ==
            (LC_WRT_CDS_CREATED | LC_ART_CDS_CREATED | LC_APP_CDS_CREATED |
             LC_WRT_CDS_RESTORED | LC_ART_CDS_RESTORED | LC_APP_CDS_RESTORED),
        "LC_OperData.TableResults == (LC_WRT_CDS_CREATED | LC_ART_CDS_CREATED "
        "| LC_APP_CDS_CREATED | LC_WRT_CDS_RESTORED | LC_ART_CDS_RESTORED | "
        "LC_APP_CDS_RESTORED)");

    // FIXME: This depends on a preprocessor constant value having a different
    // value than default.  The comparison is not done with a #if - should it?
    //    UtAssert_True(LC_AppData.CurrentLCState == LC_STATE_WHEN_CDS_RESTORED,
...
void LC_CreateTaskCDS_Test_WRTRegisterCDSError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_WRT_CDS_REGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateTaskCDS_Test_ARTRegisterCDSError(void) {
    ...
    /* Verify results */
    UtAssert_True(Result == -1, "Result == -1");
    UtAssert_True(LC_OperData.TableResults == LC_WRT_CDS_CREATED,
                  "LC_OperData.TableResults == LC_WRT_CDS_CREATED");

    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_ART_CDS_REGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_CreateTaskCDS_Test_AppDataRegisterCDSError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_APP_CDS_REGISTER_ERR_EID, CFE_EVS_ERROR,
...
void LC_LoadDefaultTables_Test_NominalActiveCDS(void) {
    ...
    /* Set to satisfy all instances of condition "Result == CFE_SUCCESS" after
     * calls to CFE_TBL_Load */
    Ut_CFE_TBL_SetReturnCode(UT_CFE_TBL_LOAD_INDEX, CFE_SUCCESS, 1);
    Ut_CFE_TBL_ContinueReturnCodeAfterCountZero(UT_CFE_TBL_LOAD_INDEX);

    /* Set to satisfy all instances of condition "Result == CFE_SUCCESS" after
     * calls to CFE_TBL_GetAddress */
    ...
    UtAssert_True(
        LC_OperData.TableResults ==
            (LC_WDT_DEFAULT_TBL | LC_ADT_DEFAULT_TBL | LC_WRT_DEFAULT_DATA |
             LC_ART_DEFAULT_DATA | LC_APP_DEFAULT_DATA | LC_CDS_UPDATED),
        "LC_OperData.TableResults == (LC_WDT_DEFAULT_TBL | LC_ADT_DEFAULT_TBL "
        "| LC_WRT_DEFAULT_DATA | LC_ART_DEFAULT_DATA | LC_APP_DEFAULT_DATA | "
        "LC_CDS_UPDATED)");
...
void LC_LoadDefaultTables_Test_ActiveCDSNotSuccess(void) {
    ...
    /* Set to make subfunction LC_UpdateTaskCDS return -1, in order to satisfy
     * condition "LC_UpdateTaskCDS() == CFE_SUCCESS" */
...
void LC_LoadDefaultTables_Test_LoadWDTError(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_WDT_LOAD_ERR_EID, CFE_EVS_ERROR,
...
void LC_LoadDefaultTables_Test_GetWDTAddressError(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_WDT_GETADDR_ERR_EID, CFE_EVS_ERROR,
...
void LC_LoadDefaultTables_Test_LoadADTError(void) {
    ...
    /* Set to satisfy condition "Result == CFE_SUCCESS" after first call to
     * CFE_TBL_Load */
    Ut_CFE_TBL_SetReturnCode(UT_CFE_TBL_LOAD_INDEX, CFE_SUCCESS, 1);
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(
            LC_ADT_LOAD_ERR_EID, CFE_EVS_ERROR,
...
void LC_LoadDefaultTables_Test_GetADTAddressError(void) {
    ...
    /* Verify results */
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_ADT_GETADDR_ERR_EID, CFE_EVS_ERROR,
...
void LC_TableInit_Test_CreateResultTablesError(void) {
    ...
    /* Set to make subfunction LC_CreateResultTables return -1, in order to
     * satisfy condition "Result = LC_CreateResultTables()) != CFE_SUCCESS" */
    Ut_CFE_TBL_SetReturnCode(UT_CFE_TBL_REGISTER_INDEX, -1, 1);
...
void LC_TableInit_Test_CreateDefinitionTablesError(void) {
    ...
    /* Set to make subfunction LC_CreateDefinitionTables return -1, in order to
     * satisfy condition "Result = LC_CreateDefinitionTables()) != CFE_SUCCESS"
     */
...
void LC_TableInit_Test_LoadDefaultTables(void) {
    ...
    /* Set to make subfunction LC_LoadDefaultTables return -1, in order to
     * satisfy condition "Result = LC_LoadDefaultTables()) != CFE_SUCCESS" */
    Ut_CFE_TBL_SetReturnCode(UT_CFE_TBL_LOAD_INDEX, -1, 1);
...
void LC_TableInit_Test_GetWDTAddressError(void) {
    ...
    LC_OperData.TableResults = LC_WRT_CDS_RESTORED | LC_ART_CDS_RESTORED |
                               LC_APP_CDS_RESTORED | LC_WDT_TBL_RESTORED |
                               LC_ADT_TBL_RESTORED;
...
void LC_TableInit_Test_CreateTaskCDSError(void) {
    ...
    /* Set to make subfunction LC_CreateTaskCDS return -1, in order to satisfy
     * condition "LC_CreateTaskCDS" */
    Ut_CFE_ES_SetReturnCode(UT_CFE_ES_REGISTERCDS_INDEX, -1, 1);
...
void LC_TableInit_Test_CDSRestored(void) {
    ...
    UtAssert_True(Ut_CFE_EVS_EventSent(
                      LC_CDS_RESTORED_INF_EID, CFE_EVS_INFORMATION,
...
void LC_TableInit_Test_CDSUpdated(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_CDS_UPDATED_INF_EID, CFE_EVS_INFORMATION,
                             "Default state loaded and written to CDS, "
                             "activity mask = 0x00991999"),
...
void LC_TableInit_Test_NeitherRestoredNorUpdated(void) {
    ...
    UtAssert_True(
        Ut_CFE_EVS_EventSent(LC_CDS_DISABLED_INF_EID, CFE_EVS_INFORMATION,
                             "LC use of Critical Data Store disabled, activity "
                             "mask = 0x00991991"),
...
void LC_App_Test_AddTestCases(void) {
    ...
    UtTest_Add(LC_AppMain_Test_Nominal, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppMain_Test_Nominal");
#endif

    UtTest_Add(LC_AppMain_Test_RegisterAppError, LC_Test_Setup,
               LC_Test_TearDown, "LC_AppMain_Test_RegisterAppError");

#ifndef LC_SAVE_TO_CDS /* default config unit test */
    UtTest_Add(LC_AppMain_Test_AppInitError, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppMain_Test_AppInitError");
    UtTest_Add(LC_AppMain_Test_RcvMsgError, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppMain_Test_RcvMsgError");
#endif

#ifndef LC_SAVE_TO_CDS /* default config unit test */
    UtTest_Add(LC_AppInit_Test_Nominal, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppInit_Test_Nominal");
#endif

    UtTest_Add(LC_AppInit_Test_EVSInitError, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppInit_Test_EVSInitError");
    UtTest_Add(LC_AppInit_Test_SBInitError, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppInit_Test_SBInitError");
    UtTest_Add(LC_AppInit_Test_TableInitError, LC_Test_Setup, LC_Test_TearDown,
               "LC_AppInit_Test_TableInitError");

    UtTest_Add(LC_EvsInit_Test_Nominal, LC_Test_Setup, LC_Test_TearDown,
               "LC_EvsInit_Test_Nominal");
    UtTest_Add(LC_EvsInit_Test_EVSRegisterError, LC_Test_Setup,
               LC_Test_TearDown, "LC_EvsInit_Test_EVSRegisterError");

    UtTest_Add(LC_SbInit_Test_Nominal, LC_Test_Setup, LC_Test_TearDown,
               "LC_SbInit_Test_Nominal");
    UtTest_Add(LC_SbInit_Test_CreatePipeError, LC_Test_Setup, LC_Test_TearDown,
               "LC_SbInit_Test_CreatePipeError");
    UtTest_Add(LC_SbInit_Test_SubscribeHKReqError, LC_Test_Setup,
               LC_Test_TearDown, "LC_SbInit_Test_SubscribeHKReqError");
    UtTest_Add(LC_SbInit_Test_SubscribeGndCmdError, LC_Test_Setup,
               LC_Test_TearDown, "LC_SbInit_Test_SubscribeGndCmdError");
    UtTest_Add(LC_SbInit_Test_SubscribeSampleCmdError, LC_Test_Setup,
               LC_Test_TearDown, "LC_SbInit_Test_SubscribeSampleCmdError");

#ifdef LC_SAVE_TO_CDS /* Alt confg unit tests */
    UtTest_Add(LC_TableInit_Test_CreateTaskCDSError, LC_Test_Setup,
               LC_Test_TearDown, "LC_TableInit_Test_CreateTaskCDSError");
    UtTest_Add(LC_TableInit_Test_CDSRestored, LC_Test_Setup, LC_Test_TearDown,
               "LC_TableInit_Test_CDSRestored");
    UtTest_Add(LC_TableInit_Test_CDSUpdated, LC_Test_Setup, LC_Test_TearDown,
               "LC_TableInit_Test_CDSUpdated");
#endif

    UtTest_Add(LC_TableInit_Test_CreateResultTablesError, LC_Test_Setup,
               LC_Test_TearDown, "LC_TableInit_Test_CreateResultTablesError");

#ifndef LC_SAVE_TO_CDS /* default config unit test */
    UtTest_Add(LC_TableInit_Test_CreateDefinitionTablesError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_TableInit_Test_CreateDefinitionTablesError");
#endif

    UtTest_Add(LC_TableInit_Test_LoadDefaultTables, LC_Test_Setup,
               LC_Test_TearDown, "LC_TableInit_Test_LoadDefaultTables");
    UtTest_Add(LC_TableInit_Test_GetWDTAddressError, LC_Test_Setup,
               LC_Test_TearDown, "LC_TableInit_Test_GetWDTAddressError");
    UtTest_Add(LC_TableInit_Test_GetADTAddressError, LC_Test_Setup,
               LC_Test_TearDown, "LC_TableInit_Test_GetADTAddressError");

    UtTest_Add(LC_CreateResultTables_Test_Nominal, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateResultTables_Test_Nominal");
    UtTest_Add(LC_CreateResultTables_Test_WRTRegisterError, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateResultTables_Test_WRTRegisterError");
    UtTest_Add(LC_CreateResultTables_Test_WRTGetAddressError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateResultTables_Test_WRTGetAddressError");
    UtTest_Add(LC_CreateResultTables_Test_ARTRegisterError, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateResultTables_Test_ARTRegisterError");
    UtTest_Add(LC_CreateResultTables_Test_ARTGetAddressError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateResultTables_Test_ARTGetAddressError");

    UtTest_Add(LC_CreateDefinitionTables_Test_NominalCDSActive, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateDefinitionTables_Test_NominalCDSActive");
    UtTest_Add(LC_CreateDefinitionTables_Test_NominalCDSInactive, LC_Test_Setup,
               LC_Test_TearDown,
    ...
    UtTest_Add(LC_CreateDefinitionTables_Test_WDTReRegisterError, LC_Test_Setup,
               LC_Test_TearDown,
    ...
    UtTest_Add(LC_CreateDefinitionTables_Test_WDTRegisterError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateDefinitionTables_Test_WDTRegisterError");
    UtTest_Add(LC_CreateDefinitionTables_Test_ADTRegisterError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateDefinitionTables_Test_ADTRegisterError");

    UtTest_Add(LC_CreateTaskCDS_Test_NominalPowerOnReset, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateTaskCDS_Test_NominalPowerOnReset");
    UtTest_Add(
        LC_CreateTaskCDS_Test_NominalProcessorReset, LC_Test_Setup,
        LC_Test_TearDown,
        "LC_CreateTaskCDS_Test_NominalProcessorReset");   // FIXME: need #if
                                                          // statement around
                                                          // this?  Depends on a
                                                          // DCR.  Currently
                                                          // skips a line.
    UtTest_Add(LC_CreateTaskCDS_Test_WRTRegisterCDSError, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateTaskCDS_Test_WRTRegisterCDSError");
    UtTest_Add(LC_CreateTaskCDS_Test_ARTRegisterCDSError, LC_Test_Setup,
               LC_Test_TearDown, "LC_CreateTaskCDS_Test_ARTRegisterCDSError");
    UtTest_Add(LC_CreateTaskCDS_Test_AppDataRegisterCDSError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_CreateTaskCDS_Test_AppDataRegisterCDSError");

    UtTest_Add(LC_LoadDefaultTables_Test_NominalActiveCDS, LC_Test_Setup,
               LC_Test_TearDown, "LC_LoadDefaultTables_Test_NominalActiveCDS");
    UtTest_Add(LC_LoadDefaultTables_Test_ActiveCDSNotSuccess, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_LoadDefaultTables_Test_ActiveCDSNotSuccess");
    UtTest_Add(LC_LoadDefaultTables_Test_NominalInactiveCDS, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_LoadDefaultTables_Test_NominalInactiveCDS");
    UtTest_Add(LC_LoadDefaultTables_Test_LoadWDTError, LC_Test_Setup,
               LC_Test_TearDown, "LC_LoadDefaultTables_Test_LoadWDTError");
    UtTest_Add(LC_LoadDefaultTables_Test_GetWDTAddressError, LC_Test_Setup,
               LC_Test_TearDown,
               "LC_LoadDefaultTables_Test_GetWDTAddressError");
    UtTest_Add(LC_LoadDefaultTables_Test_LoadADTError, LC_Test_Setup,
               LC_Test_TearDown, "LC_LoadDefaultTables_Test_LoadADTError");
    UtTest_Add(LC_LoadDefaultTables_Test_GetADTAddressError, LC_Test_Setup,
               LC_Test_TearDown,
